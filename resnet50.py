...
class BottleNeck(torch.nn.Module):
  def __init__(self, IN, OUT, S, change_channels=False):
    ...
    self.bottleneck = torch.nn.Sequential(
      ...
      torch.nn.BatchNorm2d(OUT),
      torch.nn.ReLU(inplace=True),
      torch.nn.Conv2d(in_channels=OUT, out_channels=OUT*4, kernel_size=1, stride=1, bias=False),
      torch.nn.BatchNorm2d(OUT*4)
      )
    if self.change_channels:
      self.Res= torch.nn.Sequential(
        torch.nn.Conv2d(in_channels=IN, out_channels=OUT*4, kernel_size=1, stride=S, bias=False),
        torch.nn.BatchNorm2d(OUT*4)
        )
...
def copy_res(IN, OUT, S, cnt):
  net = []
  net.append(BottleNeck(IN, OUT, S, change_channels=True))
  for i in range(1, cnt):
    net.append(BottleNeck(OUT*4, OUT, 1, change_channels=False))
...
class Net(torch.nn.Module):
   def __init__(self):
       ...
       self.conv2_x = copy_res(64, 64, S=1, cnt=3)
       self.conv3_x = copy_res(256, 128, S=2, cnt=4)
       self.conv4_x = copy_res(512, 256, S=2, cnt=6)
       self.conv5_x = copy_res(1024, 512, S=2, cnt=3)

       self.average_pool = torch.nn.AvgPool2d(7, stride=1)
       self.FC = torch.nn.Linear(2048, 10) #1*1*2048-1*10
