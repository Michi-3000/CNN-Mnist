...
class Net(torch.nn.Module):
   def __init__(self):
       ...
       self.conv1 = torch.nn.Sequential(
           torch.nn.Conv2d(in_channels=3, out_channels=64, kernel_size=7, stride=1, padding=3, bias=False), #28*28*3-28*28*64
           torch.nn.BatchNorm2d(64),
           torch.nn.ReLU(inplace=True),
           torch.nn.MaxPool2d(kernel_size=3, stride=2, padding=1)#28*28*64-14*14*64
       )
       self.conv2_x = copy_res(64, 64, S=1, cnt=2)#14*14*64
       self.conv3_x = copy_res(64, 128, S=1, cnt=2)#14*14*128
       self.conv4_x = copy_res(128, 256, S=2, cnt=2)#7*7*256
       self.conv5_x = copy_res(256, 512, S=2, cnt=2)#4*4*512

       self.average_pool = torch.nn.AvgPool2d(4, stride=1) #512
       self.FC = torch.nn.Linear(512, 10) #1*1*512-1*10
   
   # 前向计算（输入为x）
   def forward(self, x):
       #print(len(x[0]),len(x[0][0]))
       conv1_out = self.conv1(x)
       #print(len(conv1_out[0]),len(conv1_out[0][0]))
       conv2_out = self.conv2_x(conv1_out)
       #print(len(conv2_out[0]),len(conv2_out[0][0]))
       conv3_out = self.conv3_x(conv2_out)
       #print(len(conv3_out[0]),len(conv3_out[0][0]))
       conv4_out = self.conv4_x(conv3_out)
       #print(len(conv4_out[0]),len(conv4_out[0][0]))
       conv5_out = self.conv5_x(conv4_out)
       #print(len(conv5_out[0]),len(conv5_out[0][0]))
